...
suite('Command Handler', () => {
    ...
    suite('runOnServer', () => {
        ...
        const uri: vscode.Uri = vscode.Uri.parse('path');
        let addDeploymentStub: sinon.SinonStub;
        let publishStub: sinon.SinonStub;

        setup(() => {
            addDeploymentStub = sandbox.stub(serverExplorer, 'addDeployment').resolves(ProtocolStubs.okStatus);
            publishStub = sandbox.stub(serverExplorer, 'publish').resolves(ProtocolStubs.okStatus);
        });

        test('error if server explorer is not defined', async () => {
            const nullHandler = new CommandHandler(null);

            try {
                await nullHandler.runOnServer(uri);
                expect.fail();
            } catch (err) {
                expect(err).equals('Runtime Server Protocol (RSP) Server is starting, please try again later.');
            }
        ...
        test('check if rsp server is called with right param', async () => {
            const selectRSPStub = sandbox.stub(handler, 'selectRSP' as any);
            await handler.runOnServer(uri);
            expect(selectRSPStub).calledOnceWith('Select RSP provider you want to retrieve servers');
        });

        test('check if workflow is stopped if no rsp server has been chosen by user', async () => {
            sandbox.stub(handler, 'selectRSP' as any);
            const selectServerStub = sandbox.stub(handler, 'selectServer' as any);
            await handler.runOnServer(uri);
            sandbox.assert.notCalled(selectServerStub);
        });

        test('check if select Server is called with right params if rsp server has been chosen', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            const selectServerStub = sandbox.stub(handler, 'selectServer' as any);
            await handler.runOnServer(uri);
            expect(selectServerStub).calledOnceWith('id', 'Select server you want to retrieve info about');
        ...
        test('check if workflow is stopped if no server is chosen', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any);
            const getServerStateStub = sandbox.stub(serverExplorer, 'getServerStateById');
            await handler.runOnServer(uri);
            sandbox.assert.notCalled(getServerStateStub);
        });

        test('check if getServerStateById method is called correctly if both rsp and server are chosen', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any).resolves('id');
            const getServerStateStub = sandbox.stub(serverExplorer, 'getServerStateById').returns(ProtocolStubs.stoppedServerState);
            sandbox.stub(handler, 'startServer').resolves(ProtocolStubs.okStatus);
            await handler.runOnServer(uri, ServerState.RUN_MODE_RUN);
            expect(getServerStateStub).calledOnceWith('id', 'id');
        });

        test('check if addDeployment method is called correctly with right params', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any).resolves('id');
            sandbox.stub(serverExplorer, 'getServerStateById').returns(ProtocolStubs.stoppedServerState);
            sandbox.stub(handler, 'startServer').resolves(ProtocolStubs.okStatus);
            await handler.runOnServer(uri, ServerState.RUN_MODE_RUN);
            expect(addDeploymentStub).calledOnceWith([uri], ProtocolStubs.stoppedServerState);
        });

        test('check if publish method is called correctly with right params', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any).resolves('id');
            sandbox.stub(serverExplorer, 'getServerStateById').returns(ProtocolStubs.stoppedServerState);
            sandbox.stub(handler, 'startServer');
            await handler.runOnServer(uri, ServerState.RUN_MODE_RUN);
            expect(publishStub).calledOnceWith('id', ProtocolStubs.serverHandle, ServerState.PUBLISH_FULL);
        });

        test('check if startServer method is called correctly with right params if server state is stopped and server run mode is run', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any).resolves('id');
            sandbox.stub(serverExplorer, 'getServerStateById').returns(ProtocolStubs.stoppedServerState);
            const startServerStub = sandbox.stub(handler, 'startServer');
            await handler.runOnServer(uri, ServerState.RUN_MODE_RUN);
            expect(startServerStub).calledOnceWith(ServerState.RUN_MODE_RUN, ProtocolStubs.stoppedServerState);
        });

        test('check if debugServer method is called correctly with right params if server state is stopped and server run mode is debug', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any).resolves('id');
            sandbox.stub(serverExplorer, 'getServerStateById').returns(ProtocolStubs.stoppedServerState);
            const debugServerStub = sandbox.stub(handler, 'debugServer');
            await handler.runOnServer(uri, ServerState.RUN_MODE_DEBUG);
            expect(debugServerStub).calledOnceWith(ProtocolStubs.stoppedServerState);
        });

        test('check if restartServer method is called correctly with right params if server state is started and server run mode and mode passed as params are not both set to run', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any).resolves('id');
            sandbox.stub(serverExplorer, 'getServerStateById').returns(ProtocolStubs.startedServerState);
            const restartServerStub = sandbox.stub(handler, 'restartServer');
            await handler.runOnServer(uri, ServerState.RUN_MODE_DEBUG);
            expect(restartServerStub).calledOnceWith(ServerState.RUN_MODE_DEBUG, ProtocolStubs.startedServerState);
        });

        test('error if server state is not STARTED/STOPPED/UNKNOWN', async () => {
            sandbox.stub(handler, 'selectRSP' as any).resolves({id: 'id', label: 'rsp'});
            sandbox.stub(handler, 'selectServer' as any).resolves('id');
            sandbox.stub(serverExplorer, 'getServerStateById').returns(ProtocolStubs.startingServerState);
            try {
                await handler.runOnServer(uri, ServerState.RUN_MODE_DEBUG);
                expect.fail();
            } catch (ex) {
                expect(ex).equals('Unable to add distribution and run it on server id. Stop/start the server and try again.')
            }
        });
    });
